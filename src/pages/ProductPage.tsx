...
import { ArrowRight, Bot, Zap, TrendingUp, ShieldCheck, MessageSquare } from 'lucide-react';
...
function ChatSimulationArtifact() {
  ...
  return (
    <div ref={ref} className="bg-white rounded-3xl p-6 md:p-8 border border-black/[0.07] h-[400px] flex flex-col relative overflow-hidden">
      <div className="flex items-center gap-4 mb-6 pb-4 border-b border-black/[0.06]">
        ...
        <div>
          <h4 className="font-outfit font-semibold text-dark-card tracking-tight">Arvo Bot</h4>
      ...
      <div className="flex-1 overflow-hidden flex flex-col gap-4">
        {step >= 1 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3">
            ...
            <div className="bg-accent/5 text-dark-card text-sm p-3 rounded-2xl rounded-tl-sm max-w-[85%] font-light">
              Hi there! Are you looking to scale your digital presence this year?
            </div>
          </motion.div>
        )}
        {step >= 2 && (
        ...
        {step >= 3 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3">
            ...
            <div className="bg-accent/5 text-dark-card text-sm p-3 rounded-2xl rounded-tl-sm max-w-[85%] font-light">
              Perfect. Our Growth package sounds like a great fit. What's your estimated budget for this project?
            </div>
          </motion.div>
        )}
        {step >= 4 && (
          <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="absolute bottom-6 left-6 right-6">
            <div className="bg-dark-card text-white p-4 rounded-xl border border-white/10 shadow-xl flex items-center justify-between">
              <div>
                <p className="text-[10px] font-mono text-accent uppercase tracking-widest mb-1">Lead Captured</p>
...
function FeaturesArtifact() {
  ...
  const features = [
    { title: 'Neural Intent Extraction', desc: 'Identifies buyer requirements and budgets automatically.', icon: <Zap className="w-5 h-5 text-yellow-500" /> },
    { title: 'C.L.O.S.E.R Framework', desc: 'Qualifies leads using proven, high-converting sales methodologies.', icon: <TrendingUp className="w-5 h-5 text-accent" /> },
    { title: 'Custom Personas', desc: 'Agent names, titles, and avatars that build instant brand trust.', icon: <ShieldCheck className="w-5 h-5 text-purple-500" /> },
    { title: '45-Second Deployment', desc: 'Copy one line of HTML code. No backend or coding required.', icon: <MessageSquare className="w-5 h-5 text-green-500" /> },
  ];
    ...
    <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {features.map((f, i) => (
        ...
        >
          ...
          <h4 className="font-outfit font-semibold text-dark-card mb-2 tracking-tight">{f.title}</h4>
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        >
          ...
          <h1 className="text-5xl md:text-8xl font-semibold mb-8 tracking-tighter leading-[0.9] text-dark-card">
            The AI Sales Closer<br />
            <span className="font-artistic text-accent italic">that never sleeps.</span>
          </h1>
          <p className="text-xl text-text-secondary leading-relaxed max-w-2xl font-light mb-10">
            ...
            Arvo Bot lives on your site as a high-authority sales agent—qualifying prospects, 
            extracting niches, and capturing intent with neural precision.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-24">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-semibold tracking-tighter mb-8 leading-[0.95] text-dark-card">
              ...
              <span className="font-artistic text-accent italic">precision sales operation.</span>
            </h2>
            <FeaturesArtifact />
          </motion.div>
        </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10 text-center md:text-left">
            <div>
              <h3 className="text-3xl md:text-5xl font-semibold text-card-text tracking-tighter mb-4 leading-tight">
                Ready to unleash<br />
                <span className="font-artistic text-accent italic">the Beast?</span>
              </h3>
              <p className="text-card-muted font-light max-w-md">
                Deployment takes 45 seconds. Conversion lasts forever. No coding required.
              </p>
            </div>
